...
>文件描述符：在Unix系统中，一切皆文件。**文件描述符是内核为了高效操作已打开的文件的所创建的索引，是一个非负整数，用于指代被打开的文件，所有执行I/O操作的系统调用（包括Socket）都通过文件描述符。**
>
...
- 简单说一下服务器使用的并发模型？
  ...
  >本项目采用线程池，Epoll I/O模型，非阻塞Socket，同步I/O模拟的Proactor事件处理模式组成的并发模型
  >
  >
